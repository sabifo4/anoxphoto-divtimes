...
# there are 700 taxa (700), so the number of internal nodes
# is `n_taxa=700-1=699`.
...
names( mcmc_priors ) <- c( paste( "CLK_", dat, sep = "" ) )
count <- 0
for( i in c( path_prior) ){
  ...
  mcmc_priors[[count]] <- load_dat( mcmc = paste( i, "mcmc.txt", sep = "" ),
                                    ...
                                    def_samples = def_samples[count], prior = TRUE )
}
