...
# (i.e., 701-2=699) and this will be your number of divergence times that are 
...
out_file_pdf  <- sort( c( paste( "Convergence_plot_post_", dat, "_GBM",
                          ...
                          paste( "Convergence_plot_post_", dat, "_ILN",
                                 sep = "" ) ) )
out_title_pdf <- paste( c( "GBM_", "ILN_" ), sort( rep( dat, 2 ) ), sep = "" )
th            <- 0.1
...
##> good to have this object here in case some of the dirs are discarded
##> a priori because of prior knowledge about some chains being problematic
num_dirs_use <- list( 16, 16, 16, 16 )
for( i in 1:length( dataset ) ){
  # NOTE: Only vectors with more than one element rely on `i`
  sum_post_QC[[ i ]] <- QC_conv( num_dirs = num_dirs_use[[i]], delcol = delcol, 
