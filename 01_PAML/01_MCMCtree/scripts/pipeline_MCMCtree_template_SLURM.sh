...
#SBATCH --time=TAGTIME          # Runtime in HH:MM:SS
...
cd $main_dir/MCMCtree/$SLURM_ARRAY_TASK_ID/CLK/$dir
home_dir=$( pwd )
...
exec 3>&1> >(while read line; do echo "$line" >> $pipeline_dir/log.MCMCtree.dir$dir"_r"$SLURM_ARRAY_TASK_ID".txt"; done;) 2>&1
start=`date`
...
cd $home_dir
# Copy control file
cp $ctl_dir/prepcodeml.ctl $home_dir/mcmctree_$dir"_r"$SLURM_ARRAY_TASK_ID".ctl"
sed -i "s/ALN/${path_aln}/" $home_dir/mcmctree_$dir"_r"$SLURM_ARRAY_TASK_ID".ctl"
sed -i "s/TREE/${path_tree}/" $home_dir/mcmctree_$dir"_r"$SLURM_ARRAY_TASK_ID".ctl"
sed -i 's/usedata..*/usedata\ \=\ 2\ \.\/in\.BV/' $home_dir/mcmctree_$dir"_r"$SLURM_ARRAY_TASK_ID".ctl"
sed -i 's/\ ndata..*/\ ndata\ \=\ NUMPARTS/' $home_dir/mcmctree_$dir"_r"$SLURM_ARRAY_TASK_ID".ctl"
sed -i "s/lg\.dat/${path_lgdat}/" $home_dir/mcmctree_$dir"_r"$SLURM_ARRAY_TASK_ID".ctl"
if [[ CLK =~ "GBM" ]]
then 
sed -i 's/clock..*/clock\ \=\ 3/' $home_dir/mcmctree_$dir"_r"$SLURM_ARRAY_TASK_ID".ctl"
elif [[ CLK =~ "ILN" ]]
then 
sed -i 's/clock..*/clock\ \=\ 2/' $home_dir/mcmctree_$dir"_r"$SLURM_ARRAY_TASK_ID".ctl"
fi 
...
if [[ $checkp -eq 1 ]]
then 
printf "\ncheckpoint = 1 \n" >> $home_dir/mcmctree_$dir"_r"$SLURM_ARRAY_TASK_ID".ctl"
fi
